...
class XMLParser:
	def __init__(self):
		...
		self.index_dict = {} #dictionary mapping values from dict to data tags as per semantic standard (MTConnect) - used to write headers in the CSV output
	...
	def output(self, node, num):
		...
		for child in children: #if node has children then traverse through all the children
			a, val = self.output(child, 0) #recursive function to look at each child node
	...
	def publish(self, node, val):
		...
		if dataItemId not in self.index_dict.keys(): #if a new data attribute is found then update index_dict
			self.index_dict.update({dataItemId:len(self.index_dict)})
		if timestamp not in self.dict.keys(): #if a new timestamp is found, then create a entry with the key being the timestamp in dict
			entry = {timestamp: {dataItemId: str(val)}}
			self.dict.update(entry) #update entry
		else: #if it is an existing timestamp or data attribute then update dict
			if dataItemId not in self.dict[timestamp].keys():
				entry = {dataItemId: str(val)}
				self.dict[timestamp].update(entry)
			else:
				pass

	def parse(self, file):
		doc = xml.dom.minidom.parseString(file) #create node tree to represent xml file
		x,y = self.output(doc, 0) #parse through xml tree and populate dictionaries
		w = csv.writer(open("data.csv", "w", newline = "")) #write to csv output
		header = ["timestamp"] #header for csv file
		for key in self.index_dict.keys():
			header.append(key)
		w.writerow(header) #write header to csv file

		for key in self.dict.keys(): #write values from dict to csv using index_dict to map values to header
			if not key:
				continue
			temp_dict = self.dict[key]
			csv_output = [None]*(len(self.index_dict)+1)
			csv_output[0] = key
			for nested_key in temp_dict:
				csv_output[self.index_dict[nested_key]+1] = temp_dict[nested_key]
